# VOID FLAPGAME

definitely an original game that i came up with entirely on my own.

definitely did not look at another extremely similar game and go "yeah imma make that."

you play as some square thing that is still called `bird.png` for some reason and try to fly through neon buildings without dying.

## how to play

- press space or click to flap
- fly through the gaps
- don't hit the buildings
- don't hit the ground
- survive for as long as you can
- if you die, skill issue

## about

made this mostly for fun while messing around with html, css, and javascript.

all the designs were made by yours truly in paint. yes, every questionable artistic decision you see here is mine.

also yeah, the obstacles are buildings btw. i was too lazy to properly crop the shit so just deal with it.

and no, i did not make absolutely everything from scratch. looked some stuff up and got some help with the code because i am not suffering through all of that alone.

but the designs, ideas, and questionable decisions are mine.

## inspiration

none.

absolutely none.

this is clearly the first game in human history where you click to make something fly through gaps between obstacles.

the game mechanics being suspiciously familiar? coincidental.

the way the character moves? coincidental.

the obstacles coming from the top and bottom? coincidental.

the scoring system? coincidental.

the animations looking familiar too? also coincidental. i swear.

any similarities to certain birds that may or may not flap are purely coincidental.

100% original concept.

do not investigate further.

## current features

- square thing that is apparently a bird
- neon buildings (YES those are buildings)
- questionably cropped obstacles
- all designs made by yours truly
- unnecessarily dramatic main menu
- how to play screen for an extremely complicated game
- score and saved high score
- spinning square physics
- death animation
- skill issue screen
- suspiciously familiar game mechanics
- completely coincidental animations
- questionable physics
- somehow actually playable

## mobile support

this does NOT work properly on phones btw.

tested it. found out the hard way.

it technically has touch controls, but apparently that means absolutely nothing when you're trying to run the downloaded html files on android/ios.

so yeah, laptop/desktop for now.

might make a properly hosted version eventually so it actually works on phones.

heavy emphasis on **might**.

## might add later

might add more maps, different difficulties, more characters, new obstacles, effects, animations, and whatever other random shit i think of.

again, heavy emphasis on **might**.

no promises. we'll see.
